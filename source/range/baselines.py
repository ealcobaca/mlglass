...
def create_baselines(tr_data_path, ts_data_path, output_path):
    seed = 2018
    ...
    for reg_name in regressors:
        ...
        save_obj(
            ...
            '{0}log/predictions_raw_{1}.data'.format(output_path, reg_name)
        )
    ...
    z_X_tr, z_X_ts = scaler.transform(X_tr), scaler.transform(X_ts)
    ...
    for reg_name in regressors:
        ...
        save_obj(
            [test_index, y_ts, pred],
            '{0}log/predictions_pca{1}_{2}.data'.format(
                output_path, n_comp, reg_name
            )
        )
